...
export const fetchCached = async <T>(
  ...
  // A forced fetch can start while an older one for the same key is still out. Only the
  // newest may write the cache or clear the in-flight slot, or a slow stale answer would

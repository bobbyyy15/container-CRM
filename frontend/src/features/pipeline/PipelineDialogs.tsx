    ...
    <Modal title="Record sale manually" description="For a sale that didn't go through a Quotation." onClose={onClose}>
      <form onSubmit={submit}>
        <div className="modal-body" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <div><label style={{ display: 'block', fontSize: 12, fontWeight: 600, marginBottom: 6 }}>Buying cost total</label><input className="inp" type="number" min="0" step="1" value={buyingCost} onChange={event => setBuyingCost(Number(event.target.value))} required /></div>
          <div style={{ gridColumn: '1 / -1' }}><label style={{ display: 'block', fontSize: 12, fontWeight: 600, marginBottom: 6 }}>Revenue total</label><input className="inp" type="number" min="0" step="1" value={revenue} onChange={event => setRevenue(Number(event.target.value))} required /></div>

-- A re-import of people who are already in the pipeline came back as conflicts rather than
-- duplicates, and a conflict is a row someone has to resolve by hand -- so importing the
-- same working sheet twice produced a queue of busywork that had nothing wrong with it.
--
-- Company identity is the normalized name plus country/state/city. A sheet that spells the
-- address differently, or leaves it out, therefore matches no company; the contact still
-- matches on email or phone, and the row fell into "Existing contact is linked to a
-- different company identity".
--
-- When that happens, the company the contact is already attached to is now checked by name:
-- same normalized name means it is the same company, so the row is recorded as the
-- duplicate it is. A genuinely different company name still raises the conflict.

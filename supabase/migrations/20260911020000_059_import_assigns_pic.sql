-- Imported prospects were never given an owning PIC, and every lead list filters by one
-- (listActiveLeads does .eq('pic_id', picId), enforcing the data silos). So an import
-- reported "N imported" truthfully, wrote the rows, and then nobody could see a single
-- one of them -- including the person who ran it.
--
-- Each row now gets a PIC, preferring the sheet's own PIC column when it names an active
-- PIC, and falling back to the PIC of whoever ran the import. A row whose PIC column
-- names someone who is not an active PIC is assigned to the importer rather than left
-- ownerless and invisible; the sheet value stays in source_data either way.
...
-- Prospects already imported without an owner are invisible in every list. Hand each one
-- to the PIC named in its own import row, falling back to the PIC of whoever imported it.
UPDATE public.prospect_clients prospect
SET pic_id = owner.pic_id
FROM (
  SELECT
    entry.prospect_id,
    COALESCE(
      (
        SELECT pic.id FROM public.pics pic
        WHERE pic.status = 'active'
          AND public.normalize_identity_text(pic.name)
              = public.normalize_identity_text(NULLIF(btrim(entry.raw_data->>'pic'), ''))
        LIMIT 1
      ),
      (
        SELECT pic.id FROM public.pics pic
        WHERE pic.status = 'active' AND pic.profile_id = batch.created_by
        LIMIT 1
      )
    ) AS pic_id
  FROM public.import_rows entry
  JOIN public.import_batches batch ON batch.id = entry.batch_id
  WHERE entry.prospect_id IS NOT NULL
) owner
WHERE prospect.id = owner.prospect_id
  AND prospect.pic_id IS NULL
  AND owner.pic_id IS NOT NULL;

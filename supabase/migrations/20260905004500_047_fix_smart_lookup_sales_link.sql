-- Sales link to contacts through companies/quotations; sales has no contact_id.
-- Replace the lookup again using only real foreign-key paths.
...
BEGIN
    ...
    WITH candidate_matches AS (
        SELECT co.id AS match_contact_id, NULL::UUID AS match_company_id, 1 AS priority
    ...
    best_candidate AS (
        SELECT candidate_matches.match_contact_id,

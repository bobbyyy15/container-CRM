-- A 17,000-row sheet could not be imported at all: both this function and the API schema
-- cap a call at 5,000 rows, and at roughly 2.4 ms per row even 5,000 would sit on the
-- request for about twelve seconds -- past the execution limit of the serverless function
-- the API runs on.
--
-- The client now sends a large sheet in chunks that share one batch id. This makes the
-- function additive rather than single-shot: the batch row is created by the first chunk
-- and extended by the rest, row numbers continue where the previous chunk stopped so
-- "Excel row N" still points at the right line, and the counters add up instead of being
-- overwritten by the last chunk.
...
BEGIN
    ...
    -- A large sheet is sent in several calls that share one batch id: the first creates
    -- the batch, the rest add to it, so import history shows one row per file rather than
    -- one per chunk.
    ...
    SET total_rows = public.import_batches.total_rows + EXCLUDED.total_rows,
        status = 'processing',
        completed_at = NULL;

    -- Keep "Excel row N" meaningful: continue numbering where the previous chunk stopped.
    SELECT COALESCE(max(row_number), 0) INTO v_row_offset
    FROM public.import_rows WHERE batch_id = v_batch_id;
    ...
    LOOP
        ...
        BEGIN
            ...
            IF v_company_norm IS NULL THEN
                ...
                VALUES (v_batch_id, v_item.row_number + v_row_offset, v_row, 'skipped', 'No company name in the source row');
                v_skipped := v_skipped + 1;
                CONTINUE;
            END IF;
            IF v_email_1 IS NULL AND v_email_2 IS NULL AND v_phone_1 IS NULL AND v_phone_2 IS NULL THEN
                INSERT INTO public.import_rows (batch_id, row_number, raw_data, status, reason)
                VALUES (v_batch_id, v_item.row_number + v_row_offset, v_row, 'skipped', 'No email or phone in the source row');
                v_skipped := v_skipped + 1;
                CONTINUE;
            END IF;
            ...
            IF lower(COALESCE(btrim(v_row->>'category'), '')) = 'removed' THEN
                ...
                END IF;
                INSERT INTO public.import_rows (batch_id, row_number, raw_data, status, reason)
                VALUES (v_batch_id, v_item.row_number + v_row_offset, v_row, 'removed', 'Marked Removed in source sheet');
            ...
            IF v_match_count > 1 THEN
                ...
                INSERT INTO public.import_rows (batch_id, row_number, raw_data, status, reason)
                VALUES (v_batch_id, v_item.row_number + v_row_offset, v_row, 'conflict', v_reason);
            ...
            IF public.is_pipeline_identity_removed(v_company_id, v_contact_id, v_email_1, v_email_2, v_phone_1, v_phone_2) THEN
                ...
                VALUES (v_batch_id, v_item.row_number + v_row_offset, v_row, 'removed', v_reason, v_company_id, v_contact_id);
                v_removed := v_removed + 1;
                CONTINUE;
            END IF;
            ...
            IF v_company_id IS NOT NULL THEN
                ...
                IF v_owner_pic_id IS NOT NULL AND v_owner_pic_id IS DISTINCT FROM v_pic_id THEN
                    ...
                    INSERT INTO public.import_rows (batch_id, row_number, raw_data, status, reason, company_id, contact_id)
                    VALUES (v_batch_id, v_item.row_number + v_row_offset, v_row, 'conflict', v_reason, v_company_id, v_contact_id);
                    v_conflicts := v_conflicts + 1;
                    CONTINUE;
                END IF;
            ...
            IF v_contact_id IS NOT NULL THEN
                ...
                IF v_prospect_id IS NOT NULL THEN
                    ...
                    VALUES (v_batch_id, v_item.row_number + v_row_offset, v_row, 'duplicate', v_reason, v_company_id, v_contact_id, v_prospect_id);
            ...
            ELSIF v_company_id IS NOT NULL THEN
                ...
                IF v_prospect_id IS NOT NULL THEN
                    ...
                    VALUES (v_batch_id, v_item.row_number + v_row_offset, v_row, 'duplicate', v_reason, v_company_id, v_prospect_id);
                    v_duplicates := v_duplicates + 1;
                    CONTINUE;
                END IF;
            END IF;
            ...
            VALUES (v_batch_id, v_item.row_number + v_row_offset, v_row, 'imported', v_company_id, v_contact_id, v_prospect_id);
        ...
        EXCEPTION WHEN OTHERS THEN
            INSERT INTO public.import_rows (batch_id, row_number, raw_data, status, reason)
            VALUES (v_batch_id, v_item.row_number + v_row_offset, v_row, 'error', SQLERRM);
            v_errors := v_errors + 1;
        END;
    END LOOP;

    -- Counters accumulate, since earlier chunks of the same upload already contributed.
    UPDATE public.import_batches SET
        status = 'completed',
        imported_rows = imported_rows + v_imported,
        duplicate_rows = duplicate_rows + v_duplicates,
        removed_rows = removed_rows + v_removed,
        conflict_rows = conflict_rows + v_conflicts,

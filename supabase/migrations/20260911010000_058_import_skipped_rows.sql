-- "Recorded for review" should mean something is wrong. Until now a row that simply had
-- no company name, or no email and no phone, was filed under status 'error' alongside
-- genuine faults -- so a messy-but-normal source sheet (a carrier census export, say)
-- reported dozens of rows "for review" that nobody could ever fix.
--
-- Incomplete source rows now get their own status, 'skipped', and their own counter.
-- 'error' goes back to meaning an unexpected failure worth a person's attention.
--
-- This also clears the rows produced by the trigger defect migration 057 fixed: every
-- insert into prospect_clients raised `record "new" has no field "status"`, so whole
-- batches were recorded as errors that were never the data's fault. Those rows are
-- deleted and the batch counters recomputed from what remains, because import history
-- is read as a worklist and that backlog is not work.
...
ALTER TABLE public.import_rows ADD CONSTRAINT import_rows_status_check
  CHECK (status IN ('imported', 'duplicate', 'removed', 'conflict', 'skipped', 'error'));

-- Reclassify the incomplete rows already on record, so old batches read the same way
-- new ones will.
UPDATE public.import_rows
SET status = 'skipped'
WHERE status = 'error'
  AND reason IN ('Missing company name', 'At least one email or phone is required');

DELETE FROM public.import_rows
WHERE status = 'error'
  AND reason LIKE '%has no field "status"%';

-- The counters are denormalized onto the batch, so recompute them from the rows rather
-- than trying to adjust each one by hand.
WITH tally AS (
  SELECT
    batch_id,
...
SET
  imported_rows = COALESCE(tally.imported, 0),
  duplicate_rows = COALESCE(tally.duplicates, 0),
  removed_rows = COALESCE(tally.removed, 0),
  conflict_rows = COALESCE(tally.conflicts, 0),
  skipped_rows = COALESCE(tally.skipped, 0),
  error_rows = COALESCE(tally.errors, 0)
FROM tally
WHERE tally.batch_id = batch.id;

-- A batch whose every row was deleted above has no tally row to join against.
UPDATE public.import_batches batch
SET imported_rows = 0, duplicate_rows = 0, removed_rows = 0,
    conflict_rows = 0, skipped_rows = 0, error_rows = 0
WHERE NOT EXISTS (SELECT 1 FROM public.import_rows entry WHERE entry.batch_id = batch.id);

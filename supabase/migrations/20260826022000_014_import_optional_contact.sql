-- A Company and a Contact are separate domain entities. Real source data
-- (e.g. FMCSA carrier census exports) commonly has a company name with no
-- named human contact yet -- that should create the company and leave the
-- contact to be filled in later, not reject the whole row. A row with no
-- company name at all is still rejected: there is nothing to file it under.

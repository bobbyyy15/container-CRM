-- 030_fix_bulk_removed_entries.sql
--
-- 029 added an OUT parameter (via RETURNS TABLE) named identity_type, which collides with
-- the actual removed_entries.identity_type column -- PL/pgSQL then can't resolve which one
-- the INSERT's column list / ON CONFLICT target means and throws "column reference
-- identity_type is ambiguous" the moment the function is actually called (CREATE FUNCTION
-- doesn't validate the body, so this only surfaced at runtime). #variable_conflict use_column
-- tells embedded SQL to prefer the table column in that situation; the PL/pgSQL assignment
-- statements that set the OUT variables are unaffected.

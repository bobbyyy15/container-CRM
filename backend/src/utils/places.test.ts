...
test('lookupCity does not overrule a state or country already given', () => {
  assert.equal(lookupCity('ontario', 'ON'), undefined);
  assert.equal(lookupCity('ontario', '', 'Canada'), undefined);
  assert.equal(lookupCity('portland', 'OR')?.state, 'OR');
  assert.equal(lookupCity('kansas city', '', 'US')?.country, 'US');
});

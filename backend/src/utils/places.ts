 ...
 */
export const isKnownCountry = (value?: unknown): boolean =>
  COUNTRY_OPTIONS.some(c => c.code === formatCountryAbbr(value));

export const lookupCity = (cityName?: unknown, knownState?: unknown, knownCountry?: unknown): CityOption | undefined => {
  if (!cityName) return undefined;
  const raw = String(cityName).trim().toLowerCase();
  if (!raw) return undefined;

  // A state or country already on the record narrows the search instead of being overruled:
  // "Ontario" typed for a Canadian company is not Ontario, California.
  const state = formatStateAbbr(knownState);
  const country = formatCountryAbbr(knownCountry);
  return CITY_DATA.find(c =>
    c.name.toLowerCase() === raw
    && (!state || c.state === state)
    && (!isKnownCountry(country) || c.country === country)
  );
};
